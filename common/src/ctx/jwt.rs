use axum::{
    extract::{FromRequestParts, Json, TypedHeader},
    http::{request::Parts, StatusCode},
    response::{IntoResponse, Response},
    RequestPartsExt,
};
//use axum::extract::TypedHeader;
use super::req::UserInfoCtx;
...
pub static KEYS: Lazy<Keys> = Lazy::new(|| {
    let secret = "jwt_secret".to_string(); //&CFG.jwt.jwt_secret;
...
pub async fn authorize(payload: AuthPayload, token_id: String) -> Result<AuthBody, AuthError> {
    ...
    let iat = Local::now();
    //TODO
    let exp = iat + Duration::minutes(60 * 60 * 24); //CFG.jwt.jwt_exp
    let claims = Claims {
    ...
    Ok(AuthBody::new(token, claims.exp, 60 * 60 * 24, token_id)) //CFG.jwt.jwt_exp
}
